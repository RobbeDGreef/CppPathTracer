...
public:
    Uber(const std::shared_ptr<Texture> &color, double roughness, bool metallic, double transmission, const std::shared_ptr<Texture> &emission, double emission_strength) : m_baseColor(color), m_roughness(roughness), m_metallic(metallic), m_transmission(transmission), m_emission(emission), m_emission_strength(emission_strength) {}
